...
(defn render-asset [{:keys [name symbol icon decimals amount] :as asset}]
  (if name ;; If no 'name' then this the dummy value used to render `add-asset`
    ;; TODO(goranjovic): refactor to remove double dispatch
    [list/touchable-item  #(do (rf/dispatch [:set :wallet-selected-asset asset])
                               (rf/dispatch [:navigate-to :wallet-my-token]))
     [react/view
      [list/item
